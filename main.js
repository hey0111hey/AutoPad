...
var otirand = [true,true,true,true,true,true,false,false];
var step_count=0;
var isKepri = false;
var boardURL = 0;
var tmpcombo = -1;
...
function init(){
  ...
  $('#drops').hide();
  $('#oti .btn').each(function(i,elem){$(elem).hide();});
  $('#result').hide();
  ...
  if(document.addEventListener){
    ...
    // マウスボタンを離すと実行されるイベント
    canvas.attachEvent("onmouseup" ,MouseUpEvent );
    // コンテキストメニューが表示される直前に実行されるイベント
  ...
  cur_mode = 'pzl';
  console.log();
  margin_width =canvas.getBoundingClientRect().left;
  margin_height=canvas.getBoundingClientRect().top; 
  canvas.width = board.width*orb_size;
  canvas.height=  board.height*orb_size;
...
function update(){
  ...
  if(cur_mode=='end'){
    $('#modes .mode').each(function(i,elem){$(elem).removeClass('selected')});
  }
  if(cur_mode=='auto'){
    if(auto_mode=='init'){
      var x =inputHand.charAt(0);
      inputHand = inputHand.substr(1);
      var y =inputHand.charAt(0);
      inputHand = inputHand.substr(2);
      mouseX=margin_width + (x)*orb_size+orb_size*0.5;
      mouseY=margin_height+ (y)*orb_size+orb_size*0.5;
      board.touchX=x-0;
      board.touchY=y-0;
      auto_mode='move'
    }
  ...
  if(cur_mode=='inLoop'){
    tmpcombo = result.combo;
    cur_mode = 'none';
    board.elace(isKepri,function(){cur_mode='movie_elace';});
    start = new Date;
  }
  if(cur_mode=='movie_fall'){
    var next  = new Date;
    if(100<= next.getTime() - start.getTime()){
      board.falling();
      if(isOtikon)board.fill_random(otirand);
      if(tmpcombo!=result.combo){
        cur_mode='inLoop';
      }else{
        cur_mode='end';
        console.log(result.hand);
        $('#result div').text((result.hand.length-3)+'手で'+result.combo+'コンボしました。');
        $('#result a').attr("href","file:///D:/Desktop/jspad-gh-pages/index.html?board="+boardURL
            +"&move="+result.hand);
        $('#result').toggle('show');
      }
    }
  }
  if(cur_mode=='movie_elace'){
    var next  = new Date;
    if(100<= next.getTime() - start.getTime()){
      board.elace_animation(result,function(){cur_mode='movie_fall';});
      start = new Date;
    }
  }
...
function render_orbs(mode){
  ...
  for(var height = 0; height < board.height ; height++){
    for(var width = 0; width < board.width ; width++){
      var x =width*orb_size ;
      var y =height*orb_size;
      // ドロップを表示する
      // つかんでいるドロップはマウスに追従させる
      if(board.cell[width][height]!== 'vanished'){
        if(board.isTouch && board.touchX == width && board.touchY == height && (mode == 'normal'|| mode =='ctw'|| mode == 'pzl'||cur_mode =='auto')){
          ctx.drawImage(Asset.images[board.cell[width][height]],mouseX-margin_width-30,mouseY-30);
        }else{
...
board.decode = function(){
  var urlb='';
  for (var height = 0; height < board.height; ++height) {
    for (var width = 0; width < board.width; ++width) {
      urlb=urlb+icolor.indexOf(board.cell[width][height]);
    }
  }
  return urlb;
}
...
board.moving = function(mode){
  ...
  if(    mouseX>= margin_width 
    ...
    if(result.hand.length==0){
      result.hand+=board.touchX;
      result.hand+=board.touchY;
      result.hand+=',';
    }
  ...
  if(board.isBefore && board.isTouch && (mode == 'normal' || mode == 'ctw' | mode == 'pzl')){
    if(board.touchX !== board.beforeX || board.touchY !== board.beforeY ){
      ...
      result.hand = result.hand+((board.touchX-board.beforeX)+(board.touchY-board.beforeY)*3+4);
    }
  }
}
...
result.init = function(){
  ...
  result.length = 0;
  result.buf = new Array();
}

board.elace_animation= function(result,callback){
  var _count = 0;
  ...
  console.log('called elace_animation1');
  for (var i = 0; i < result.buf.length; ++i) {
    console.log(result.buf[i]);
  }
  console.log('called elace_animation2');
  for (var i = 0; i < board.cell.length; ++i) {
    for (var j = 0; j < board.cell[i].length; ++j) {
      if(result.buf[0][i+1][j+1]!='vanished'){
        board.cell[i][j]='vanished';
      }
      _onLoad();
    }
  }
...
board.fill_random = function(otirand){
  console.log('called fill_random');
...
board.elace =function(isKepri,callback){
  console.log('called board.elace');
  //番兵を用意した配列を生成する関数を用意
  ...
  var _onLoad = function(){
    ...
    if(_loadCount>=29){
      history.push(board.cell);
      console.log(result.buf);
      console.log('callback is called2');
      callback();
    }
  }
  ...
  var onLoad = function(){
    loadCount++;
    if (loadCount >= board.width*board.height) {
      console.log('callback is called1');
      history.push(board.cell);
      for (var i = 0; i < result.buf.length; ++i) {
        console.log(result.buf[i]);
      }
      ...
      callback();
      //for (var i = 0; i < result.buf.length; ++i) {
      //  if(result.buf[i].length==0){
      //    result.buf.splice(i,1);
      //    i--;
      //  }
      //  _onLoad();
      //}
  ...
  for(var countW = 1; countW < board.width +1 ; countW++){
    for(var countH = 1; countH < board.height +1 ; countH++){
      ...
      (function(countW,countH){
        if(erace_state[countW][countH]=='vanished'){
          onLoad();
          return 1;
        }
        var tmp = _make_Array();
        ...
        if(isKepri&&count<=4){
          ...
          result.buf.push(_copy_Array(tmp));
          console.log(tmp);
        }
        onLoad();
        return tmp
...
function set_color(clr){
  cur_mode="orbs";
  $('#drops div').each(function(i,elem){
    $(elem).removeClass('selected');
  });
      $('#drops #'+clr).addClass('selected');
    select_color = clr;
    history.push(board.cell);
...
function onRadioButtonChange(entry){
  ...
  $('#oti .btn').each(function(i,elem){
    if(otirand[i])$(elem).addClass("use");
    else $(elem).removeClass("use");
  });
  console.log(otirand);
}
...
function isUseOtikon(){
  isOtikon = !isOtikon;
  if(isOtikon){
    $('#obtn').text("落ちコン：選択してください")
      .addClass("selected");
